...
**Assumption 2' (Ratio stable unit treatment value)**

The observed outcomes are determined by
...
\boldsymbol u_{i,d,n}
=
\boldsymbol w_{i,d}-\overline{\boldsymbol w}_{d,n},
\qquad
\mathbf S_{dd,n}
=
\frac{1}{n}\sum_{i=1}^n
\boldsymbol u_{i,d,n}\boldsymbol u_{i,d,n}'.
$$ {#eq-ratio-treatment-group-covariance}
...
\left(
p,
\{\boldsymbol{w}_{i,0}\}_{i=1}^\infty,
\{\boldsymbol{w}_{i,1}\}_{i=1}^\infty
\right),
$$
...
**Assumption 3' (Ratio regularity)**

[**(i)**]{.part-label} **(Non-degeneracy)** There exists an index $n_0$ for which $\mathbf S_{dd,n}$ is positive definite for all $n\geq n_0$, $d\in\{0,1\}$, and $\boldsymbol\theta\in\Theta$.
...
\max_{1\leq i\leq n}
\frac{
\boldsymbol u_{i,d,n}'\mathbf S_{dd,n}^{-1}\boldsymbol u_{i,d,n}
}{n}
=0.
$$
:::

Assumption 3'(ii) implies the coordinatewise no-dominant-unit condition in the treatment group means note.
...
Finally, we use a denominator-strength screen to select between the two intervals.
The resulting hybrid interval is uniformly asymptotically valid: it defaults to the Fieller difference interval unless the screen certifies both denominators as strong, in which case it uses the tighter delta-method interval.

## Fieller difference confidence interval

### Treatment-group-specific Fieller intervals

We first use Fieller's method to construct confidence intervals for the ratios in each treatment group.
For $d\in\{0,1\}$, define
$$
r_{d,n}
=
...
\widehat s_{v,d,n}^2(r)
$$ {#eq-single-treatment-group-v-scale}
where
$$
\begin{aligned}
\widehat s_{v,d,n}^2(r)
...
\end{aligned}
$$ {#eq-single-treatment-group-residual-variance}

For fixed $r\in\mathbb R$, the [treatment group means note](../treatment-group-means/#eq-treatment-group-wald-ci) shows that the confidence set
$$
C_{1-\alpha/2,d,n}(r)
=\left\{
v\in\mathbb R:
...
\right\},
\qquad d\in\{0,1\}.
$$ {#eq-single-treatment-group-v-interval}
...
\frac{\alpha}{2},
\qquad d\in\{0,1\}.
$$ {#eq-treatment-group-fieller-coverage}

### Difference confidence interval

We now combine the treatment-group-specific Fieller sets to construct a confidence interval for $r_n=r_{1,n}-r_{0,n}$.
To determine whether both treatment-group-specific sets are bounded, define the denominator test statistics
$$
T_{d,n}^z =
...
Otherwise, the endpoint construction contains every difference $x_1-x_0$ with $x_d\in C_{1-\alpha/2,d,n}^{\mathrm F}$, and hence contains $r_{1,n}-r_{0,n}$ whenever both treatment-group-specific sets cover their respective targets.
...
$$
Applying (-@eq-treatment-group-fieller-coverage) to the two terms on the right yields
$$
\liminf_{n\to\infty}
\inf_{\boldsymbol\theta\in\Theta}
\mathbb P_{\boldsymbol\theta}
...
The simulation suggests that the Fieller difference interval is conservative.
This is because the Bonferroni argument controls the larger event that at least one treatment-group-specific Fieller set misses its target rather than the smaller event that the Fieller difference interval misses $r_n$.
Sharpening the bound requires controlling the joint behavior of the two random sets, which leads to a difficult Gaussian interval-overlap problem.
...
$$
\sigma_{r,n}^2 = \mathbb V_{\boldsymbol\theta}\left(\frac{1}{\sqrt n}\sum_{i=1}^n X_{i,n}\right).
$$
Let
$$
s_{\ell,d,n}^2
...
\widehat s_{\ell,d,n}^2,
\qquad d\in\{0,1\},
$$ {#eq-ratio-treatment-group-scale}
and let
$$
...
Under Assumption 3'(i), $\widetilde{\sigma}_{r,n}>0$ for all $n$ sufficiently large.
We therefore define
$$
...
Thus (-@eq-ratio-oracle-normal-approximation), (-@eq-ratio-reciprocal-scale-consistency), and @prp-asymptotic-equivalence-slutsky imply
$$
\frac{1}{\sqrt n\,\widehat{\sigma}_{r,n}}
\sum_{i=1}^n X_{i,n}
\stackrel{\mathcal P}{\approx}Z_n.
$$ {#eq-ratio-feasible-leading-normal-approximation}
...
Using (-@eq-ratio-score-decomposition) and (-@eq-ratio-feasible-remainder-bounds), we can therefore write
$$
\begin{aligned}
...
Together with (-@eq-ratio-feasible-leading-normal-approximation), @prp-asymptotic-equivalence-perturbation implies
$$
\frac{\sqrt n(\widehat r_n-r_n)}{\widehat{\sigma}_{r,n}}
\stackrel{\mathcal P}{\approx}Z_n.
$$
...
Using the same argument as in the treatment group means note, we conclude that, for every sequence $\boldsymbol\theta_n\in\Theta$ satisfying (-@eq-ratio-regular-denominator-condition),
$$
\liminf_{n \to \infty}
\mathbb{P}_{\boldsymbol\theta_n}\left(r_n \in C_{1-\alpha,n}^{\mathrm{DM}}\right)
\geq 1 - \alpha
...
To show that $C_{1-\alpha,n}^{\mathrm H}$ is asymptotically valid, suppose to the contrary that there exist $\epsilon>0$, a sequence $n_k\to\infty$, and parameters $\boldsymbol\theta_k\in\Theta$ such that
$$
\mathbb P_{\boldsymbol\theta_k}
\left(
...
$$
Because $[0,1]$ is compact, there exist a strictly increasing sequence $\{k_j\}_{j\geq1}$ and $q\in[0,1]$ such that $q_{k_j}\to q$.
We will show that
$$
\limsup_{j\to\infty}
\mathbb P_{\boldsymbol\theta_{k_j}}
\left(
...
We first establish a normal approximation that is uniform over $\boldsymbol\theta\in\Theta$ and $r\in\mathbb R$.
Consider arbitrary sequences $n_k\to\infty$, $\boldsymbol\theta_k\in\Theta$, and $r_k\in\mathbb R$.
For brevity, relabel the subsequence by $n$ and write its parameters as $\boldsymbol\theta_n$ and $r_n$.
...
:=
\sum_{i=1}^n
\mathbb E_{\boldsymbol\theta_n}\left[X_{i,d,n}^2(r_n)\right]
=
np_dp_{1-d}s_{v,d,n}^2(r_n).
$$
...
Assumption 1 bounds $p_dp_{1-d}$ below by some $c>0$, so the Rayleigh-quotient inequality and Assumption 3'(ii) give
$$
\begin{aligned}
\max_{1\leq i\leq n}
\frac{X_{i,d,n}^2(r_n)}{s_{d,n}^2(r_n)}
...
$$
\boldsymbol u_{i,d,n}^{\circ}
=
\mathbf S_{dd,n}^{-1/2}\boldsymbol u_{i,d,n}.
$$
Then
$$
\frac{1}{n}\sum_{i=1}^n
\boldsymbol u_{i,d,n}^{\circ}\boldsymbol u_{i,d,n}^{\circ\prime}
=\mathbf I_2,
\qquad
\sum_{i=1}^n\boldsymbol u_{i,d,n}^{\circ}=\boldsymbol0.
$$
...
&=
\frac{p_dp_{1-d}}{n^2}
\sum_{i=1}^n\|\boldsymbol u_{i,d,n}^{\circ}\|^4 \\
&\leq
\frac{1}{2}
\max_{1\leq i\leq n}
\frac{\|\boldsymbol u_{i,d,n}^{\circ}\|^2}{n} \\[1ex]
&\to0.
\end{aligned}
$$
where convergence follows from Assumption 3'(ii).
The argument in @prp-uniform-moment-bounds(i), applied along the sequence $\boldsymbol\theta_n$ with the Frobenius norm, and the bound $\|\mathbf A\|\leq\|\mathbf A\|_{\mathrm F}$ therefore give
$$
\left\|
\frac{1}{n}\sum_{i=1}^n
(D_{i,d}-p_d)
...
\left\|
\frac{1}{n}\sum_{i=1}^n
(D_{i,d}-p_d)\boldsymbol u_{i,d,n}^{\circ}
\right\|^2
=
\frac{2p_dp_{1-d}}{n}
\leq\frac{1}{2n} \to 0.
$$ {#eq-whitened-mean-second-moment}
The argument in @prp-uniform-moment-bounds(i), again applied along $\boldsymbol\theta_n$, gives

$$
\frac{1}{n}\sum_{i=1}^n
(D_{i,d}-p_d)\boldsymbol u_{i,d,n}^{\circ}
\stackrel p\to\boldsymbol0.
$$

Now define
$$
\overline{\boldsymbol U}_{d,n}^{\circ}
=
\frac{1}{n\overline D_{d,n}}
\sum_{i=1}^nD_{i,d}\boldsymbol u_{i,d,n}^{\circ}
=
\frac{1}{\overline D_{d,n}}
\frac{1}{n}\sum_{i=1}^n
(D_{i,d}-p_d)\boldsymbol u_{i,d,n}^{\circ}.
$$
Since $\overline D_{d,n}\stackrel p\to p_d$ and $p_d$ is bounded away from zero, Slutsky's theorem gives
$$
\overline{\boldsymbol U}_{d,n}^{\circ}\stackrel p\to\boldsymbol0.
...
\frac{1}{n\overline D_{d,n}}
\sum_{i=1}^nD_{i,d}
(\boldsymbol u_{i,d,n}^{\circ}-\overline{\boldsymbol U}_{d,n}^{\circ})
(\boldsymbol u_{i,d,n}^{\circ}-\overline{\boldsymbol U}_{d,n}^{\circ})' \\
&=
...
-
\overline{\boldsymbol U}_{d,n}^{\circ}
\overline{\boldsymbol U}_{d,n}^{\circ\prime}.
\end{aligned}
$$
...
\right\|
+
\|\overline{\boldsymbol U}_{d,n}^{\circ}\|^2 \\
&
\stackrel p\to0.
\end{aligned}
...
$$
\begin{aligned}
&\left|\frac{\widehat s_{v,d,n}^2(r)}{s_{v,d,n}^2(r)}-1\right| \leq \left\|\mathbf S_{dd,n}^{-1/2}\widehat{\mathbf S}_{dd,n}\mathbf S_{dd,n}^{-1/2}-\mathbf I_2\right\|.
\end{aligned}
...
$$
\frac{\widehat s_{v,d,n}^2(r_n)}{s_{v,d,n}^2(r_n)} \stackrel p\to1.
...
If (-@eq-uniform-projected-coverage) failed, there would exist $\epsilon>0$, a subsequence $n_k\to\infty$, and choices $\boldsymbol\theta_k\in\Theta$ and $r_k\in\mathbb R$ such that
$$
\mathbb P_{\boldsymbol\theta_k}
\left(
...
But then
$$
\limsup_{k\to\infty}\mathbb P_{\boldsymbol\theta_k}
\left(
\overline v_{d,n_k}(r_k)
\in C_{1-\alpha/2,d,n_k}(r_k)
\right)
...
$$
Uniform covariance consistency and uniform tightness of the studentized denominator errors imply that, for every sequence $\boldsymbol\theta_n\in\Theta$,
$$
\liminf_{n\to\infty}
\mathbb P_{\boldsymbol\theta_n}(T_n^z\geq\tau_n)>0
\quad\Longrightarrow\quad
\min_{d\in\{0,1\}}
...
For a treatment group attaining the minimum, the triangle inequality gives
$$
...
Consider an arbitrary sequence $\boldsymbol\theta_n\in\Theta$ satisfying (-@eq-ratio-regular-denominator-condition).
For each treatment group, the reverse triangle inequality gives
$$
\frac{\sqrt n\,|\overline Z_{d,n}|}{\widehat\sigma_{z,d,n}}
\geq
\frac{\sigma_{z,d,n}}{\widehat\sigma_{z,d,n}}
...
\right).
$$
Taking the minimum over the two treatment groups yields
$$
T_n^z
\geq
\min_{d\in\{0,1\}}
...
Along $\boldsymbol\theta_n$, covariance consistency implies that the factor before the brackets converges in probability to one, (-@eq-ratio-regular-denominator-condition) makes the first term in brackets diverge, and tightness of the studentized denominator errors makes the second term $O_{\mathcal P}(1)$.
Thus $T_n^z\stackrel{\mathcal P}{\to}\infty$, and hence, for the fixed threshold $k_\alpha$ in (-@eq-nesting-threshold),
$$
\mathbb P_{\boldsymbol\theta_n}(T_n^z<k_\alpha)
\to0.
$$
...
\widehat q_{d,n}-q_{d,n}
=
\frac{L_{d,n}}{1+E_{z,d,n}}
=
L_{d,n}
-
L_{d,n}\frac{E_{z,d,n}}{1+E_{z,d,n}}.
...
For each treatment group, observe that
$$
\widehat\sigma_{\ell,d,n}
=
\frac{\widehat\sigma_{v,d,n}(\widehat q_{d,n})}
{|\overline Z_{d,n}|}.
$$ {#eq-treatment-group-delta-fieller-scale}
...
\widehat\sigma_{\ell,d,n}.
$$
On $\{T_n^z\geq k_\alpha\}$, equations (-@eq-nesting-threshold)--(-@eq-treatment-group-delta-fieller-scale) imply
$$
\begin{aligned}
&|x-\widehat q_{d,n}|
\left(
...
\subseteq
C_{1-\alpha/2,d,n}^{\mathrm F}
$$ {#eq-treatment-group-nesting}
on $\{T_n^z\geq k_\alpha\}$.
Taking pairwise differences in (-@eq-treatment-group-nesting) proves (-@eq-delta-fieller-nesting).

## Delta-method approximation {#sec-ratio-delta-approximation}

Recall the oracle scores in (-@eq-ratio-score).
Recall their population variances in (-@eq-ratio-score-variances), and define their between-treatment-group covariance by
$$
s_{\ell,01,n}
=
\frac{1}{n}\sum_{i=1}^n
...
$$
Recall the treatment-group-specific and aggregate population scales in (-@eq-ratio-treatment-group-scale) and (-@eq-ratio-robust-scale), respectively.
Because $D_{i,0}-p_0=-(D_{i,1}-p_1)$ and assignments are independent across units,
$$
\begin{aligned}
\omega_n^2
&=
\mathbb V_{\boldsymbol\theta}\left(
\frac{1}{\sqrt n\,\widetilde{\sigma}_{r,n}}
\sum_{i=1}^n X_{i,n}
\right) \\
&=
...
\end{pmatrix},
\qquad
\ell_{i,d,n}=\boldsymbol a_{d,n}'\boldsymbol u_{i,d,n}.
$$
...
\boldsymbol u_{i,d,n}.
$$
Thus Assumption 3'(ii) implies the scalar no-dominant-unit condition for both oracle score sequences.
The [normal approximation for treatment group means](../treatment-group-means/#eq-treatment-group-infeasible-scalar-normal-approximation), applied to their difference, therefore gives
$$
\frac{1}{\sqrt n\,\widetilde{\sigma}_{r,n}}
\sum_{i=1}^n X_{i,n}
\stackrel{\mathcal P}{\approx}Z_n,
...
$$
Since $\sum_{i=1}^n\ell_{i,d,n}=0$ and $D_{i,d}Y_i=D_{i,d}y_{i,d}$ and $D_{i,d}Z_i=D_{i,d}z_{i,d}$, the term for treatment group $d$ in (-@eq-ratio-linearization) is exactly
$$
\frac{1}{\sqrt n\,\overline D_{d,n}}
...
$$
Moreover, $\mathbb V_{\boldsymbol\theta}(U_{d,n})=\sigma_{\ell,d,n}^2$.
...
For $\boldsymbol c(x)=(1,-x)'$, the Rayleigh-quotient inequality gives
$$
\sup_{x\in\mathbb R}
\left|
\frac{\widehat s_{v,d,n}^2(x)}{s_{v,d,n}^2(x)}-1
\right|
\leq
\left\|
\mathbf S_{dd,n}^{-1/2}
...
+o_{\mathcal P}(1).
$$
For the treatment-group-specific linear terms defined in (-@eq-ratio-exact-remainder),
$$
\frac{\sqrt n\,L_{d,n}}{\sigma_{\ell,d,n}}
=O_{\mathcal P}(1).
$$
Equation (-@eq-ratio-exact-remainder) writes each treatment-group-specific remainder as $-L_{d,n}E_{z,d,n}/(1+E_{z,d,n})$.
...
Together with (-@eq-ratio-feasible-leading-normal-approximation), this gives a Gaussian approximation with variance $\omega_n^2\leq1$ for the feasible statistic.
