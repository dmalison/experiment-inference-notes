...
The left panel in each figure displays a fixed set of potential outcomes for the first 20 observations of a sample of $n = 1{,}000$ units.
...
In this case, the conservative bound in (-@eq-conservative-variance) coincides with the true variance. Therefore the studentized estimator closely tracks the standard normal and the empirical coverage is close to the nominal $95\%$ level.
...
The estimator variance is much lower than the bound, so the studentized estimator is under-dispersed relative to the standard normal and the interval covers $\psi_n$ more often than the nominal level.

![Coverage of the confidence interval under heterogeneous treatment effects.](heterogeneous_coverage_animation.gif){#fig-heterogeneous-coverage}
