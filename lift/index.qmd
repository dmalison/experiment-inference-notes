...
Because $\psi_n^+$ and $\psi_n^-$ are linear combinations of the treatment group means, we can apply the results from the treatment group means note. The associated coefficient vector is
$$
\boldsymbol a_n^s
=
(\rho_n^s+m^s,-m^s)',
...
$$
where $\lambda$ denotes Lebesgue measure and $\triangle$ denotes the symmetric difference between sets. When $|T_{0,n}|$ is large, the right-hand side is close to zero and the two sets are approximately equal.
