...
If we compare (-@eq-scaled-ate-interval) with (-@eq-delta-method-interval), we see that the delta method standard error accounts for denominator uncertainty by replacing the $\widehat{\sigma}_{0,n}$ with $\left|1+\operatorname{sgn}(\widehat{y}_{0,n})\widehat{\rho}_n\right|\widehat{\sigma}_{0,n}$.
This difference explains the pattern in @fig-scaled-ate-noncoverage: the intervals are similar near $\rho_n=0$, but diverge as the lift moves away from zero. Because the baseline is positive, the scaled ATE interval understates uncertainty for positive lifts and overstates it for negative lifts.
...
For example, many outcomes are known to only take nonnegative values.
Then $\rho_n=\rho_n^s$,
...
Because $\alpha^{\mathrm{baseline}}\leq\alpha^{\mathrm{Fieller}}$, rejection of the denominator sign test implies the selected sign-specific confidence set is a bounded interval.
...
The green and red lines in the bottom panel show the known-sign intervals $I_{1-\alpha,n}^{+}$ and $I_{1-\alpha,n}^{-}$.
Near zero, the uniform set and the known-sign intervals are conservative because the sign tests often return the real line.
Across the full range of baseline values, the confidence intervals keep non-inclusion below the nominal rate.
...
In the case when the sign of $B_n$ is unknown, we use the statistic $T_{0,n}^{\mathrm{full}}$ to determine which sign-specific set to use:
...
We assume in this section that $|T_n^{\mathrm{baseline}}|> z_{1-\alpha/2}$, which implies $b < 1$.
...
Since the delta-method confidence interval can be written as $\{r: |r-\widehat{\rho}_n|\leq h\}$, the sign-specific confidence set is squeezed between a contraction and expansion of the delta-method confidence interval.
Taken together, (-@eq-fieller-delta-outer-inclusion) and (-@eq-fieller-delta-inner-inclusion) imply
