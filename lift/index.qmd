...
The bottom panel plots the empirical non-coverage as a function of $\rho_n$, with the two lift values from the top panels marked in matching histogram colors.
...
This difference explains the pattern in @fig-scaled-ate-noncoverage: the intervals are similar near $\rho_n=0$, but diverge as the lift moves away from zero. Because the baseline in this simulation is positive, the scaled-ATE interval understates uncertainty for positive lifts and overstates it for negative lifts.
...
In this section, we use this result to construct a uniformly valid confidence interval for $\rho_n$.
...
Now let $T_n^{\mathrm{baseline}}$ be the test statistic for whether the baseline mean is zero:
$$
T_n^{\mathrm{baseline}}
=
...
When the sign of $\overline{y}_{0,n}$ is unknown, we can use the denominator statistic $T_n^{\mathrm{baseline}}$ in (-@eq-baseline-statistic) to determine which sign-specific set to use.
...
If $T_n^{\mathrm{baseline}}$ is significantly positive in the upper tail at level $\alpha^{\mathrm{baseline}} / 2$, $I_{1-\alpha,n}$ uses the positive-denominator set; if it is significantly negative, $I_{1-\alpha,n}$ uses the negative-denominator set.
...
@fig-uniform-noncoverage recreates the bottom panels of @fig-scaled-ate-noncoverage and @fig-lift-skewness, but reports non-coverage for the uniform set in (-@eq-final-confidence-set). The top panel varies the lift at a fixed baseline $\overline{y}_{0,n}=1$.
...
Across the full range of baseline values, the confidence intervals keep non-coverage below the nominal rate.

![Empirical non-coverage of the nominal $95\%$ uniformly valid confidence set in (-@eq-final-confidence-set), with $\alpha^{\mathrm{baseline}}=0.001$ and $\alpha^{\mathrm{Fieller}}=0.049$, along with the known-positive and known-negative baseline intervals $I_{1-\alpha,n}^{+}$ and $I_{1-\alpha,n}^{-}$ from the known-baseline-sign construction. Top: as a function of the percentage lift $\rho_n$ at fixed baseline $\overline{y}_{0,n}=1$. Bottom: as a function of the number of standard errors $\sqrt{n}\,\overline{y}_{0,n}/\sigma_{0,n}$ separating the baseline from zero at fixed lift $\rho_n=10\%$.](lift_uniform_noncoverage.png){#fig-uniform-noncoverage}
...
Throughout this section, assume that $|T_n^{\mathrm{baseline}}|> z_{1-\alpha/2}$, which implies $b < 1$.
