...
Assumption 1 excludes stratified designs that assign a fixed fraction of units to each treatment group.
Such designs are less common in online experiments, where treatment groups are typically assigned on the fly.
...
In this case, Assumption 1 holds by taking $\underline{p}$ to be the smallest permitted fraction of buckets that can be allocated to one group.

In addition to $D_i$, we observe an outcome of interest $Y_i$ which we model using potential outcomes.
Let $y_{i,d}$ denote unit $i$'s **potential outcome** under treatment $d \in \{0, 1\}$.
We treat the potential outcomes as fixed and assume the observed outcome is generated according to the following rule:
...
Assumption 2 also implies that treatment assignment is the only source of randomness in the model.
By treating the potential outcomes as fixed, we abstract from population sampling issues and conduct inference conditional on the experimental units and their potential outcomes.
...
Although these examples use coefficient vectors that are fixed across $n$, we allow $\mathbf a_n$ to vary with $n$ to accommodate other applications (e.g.~the [sign-specific contrasts in the relative treatment effects note](../lift/#splitting-by-the-control-mean-sign)).
...
We impose the following regularity conditions on $\Theta$:
...
Weaker, target-specific conditions suffice in many cases of interest.
In the [Appendix](#sec-treatment-group-mean-regularity), we show that Assumption 3(i) implies Assumption 3(ii) for a single treatment group mean.
We also show in the [Appendix](#sec-ate-regularity) that the same implication holds for the ATE as long as
$$
...
For $\theta\in\Theta$, we let $\mathbb{P}_\theta$ denote probability of the observed sample $\{(D_i, Y_i)\}_{i=1}^n$ under the random assignment mechanism in Assumption 1 and the observation equation in Assumption 2.
...
We focus on uniform validity because platforms performing online experiment analysis must produce reliable results across a wide range of data-generating processes.
...
## Regularity conditions for treatment group means and ATE  {#sec-treatment-group-mean-regularity}

**Treatment group mean.**
For a single treatment group mean under treatment state $d\in\{0,1\}$, only coordinate $d$ of $\mathbf b_n$ is nonzero.
Consequently, the quadratic form $\mathbf b_n'\mathbf V_n\mathbf b_n$ is a positive multiple of $S_{d,n}^2$ and is therefore eventually positive by Assumption 3(i).
...
Therefore, Assumption 3(ii) follows.
