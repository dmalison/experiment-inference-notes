...
Average treatment effects (ATEs) are often reported relative to a baseline mean.
This note studies robust statistical inference for that ratio.
We first show why scaling the confidence interval for the ATE or applying the delta method can yield confidence intervals with unreliable coverage.
We then construct uniformly-valid confidence intervals by combining Fieller's method with sign tests.
We also show how to adapt the construction when effects are reported relative to a full-population baseline.
^[I thank Sergey Gitlin for proposing many of the ideas developed here.]
...
\widehat{\sigma}_n^{\mathrm{ATE}} = \widehat{\sigma}_{0,n}+\widehat{\sigma}_{1,n}.
\end{gathered}
$$

Here $\widehat{\sigma}_{d,n}^2$ is the treatment-group-specific variance estimator defined in (-@eq-treatment-group-variance-estimator) of the [treatment group means note](../treatment-group-means/).
...
At $\rho_n=1\%$, the studentized statistic is close to $N(0,1)$; at $\rho_n=100\%$, however, it is visibly over-dispersed.
The bottom panel shows that non-coverage matches the nominal level only when $\rho_n=0$.
For positive lifts, the interval is too narrow; for negative lifts, the interval is too wide.
...
This difference explains the pattern in @fig-scaled-ate-noncoverage: the intervals are similar near $\rho_n=0$, but diverge as the lift moves away from zero.
...
Because $\psi_n^+$ and $\psi_n^-$ are linear combinations of the treatment group means, we can apply the results from the treatment group means note.
The associated coefficient vector is
$$
\boldsymbol a_n^s
=
(\rho_n^s+m^s,-m^s)',
...
The second coordinate of $\boldsymbol a_n^s$ is nonzero, so this coefficient vector is nonzero for every $n$ and both signs $s$.
...
\left\{\psi_n^s \in C_{1-\alpha,n}^s\right\}
\iff
\left\{\left|\frac{\widehat{\psi}_n^s - \psi_n^s}{\sqrt{(\widehat{\sigma}_n^{s}(\boldsymbol\theta))^2/n}}\right| \leq z_{1-\alpha/2}\right\},
$$
where
$$
(\widehat{\sigma}_n^{s}(\boldsymbol\theta))^2
=
...
{\sqrt{(\widehat{\sigma}_n^{s}(\boldsymbol\theta))^2/n}}
...
$$
\liminf_{n \to \infty} \inf_{\boldsymbol\theta \in \Theta} \mathbb{P}_{\boldsymbol\theta}(\rho_n^s \in C_{1-\alpha,n}^{s}) \geq 1-\alpha
$$ {#eq-sign-specific-fieller-coverage}


The expression on the left-hand side of the inequality in (-@eq-fieller-sets) is piecewise quadratic.
Each piece has leading coefficient
...
@fig-fieller-confidence-set-bounded displays the bounded case.
The value of the Fieller inequality is positive in both tails, so each sign-specific confidence set is an interval around the corresponding point estimate.
Note that the point estimate is not the midpoint of the interval.
...
@fig-fieller-confidence-set-unbounded displays the unbounded case.
The value of the Fieller inequality is negative in the tails, so the confidence set contains arbitrarily large positive or negative lifts.
...
$$
where $\lambda$ denotes Lebesgue measure and $\triangle$ denotes the symmetric difference between sets.
When $|T_{0,n}|$ is large, the right-hand side is close to zero and the two sets are approximately equal.
...
Then $\rho_n=\rho_n^s$, so the sign-specific confidence set $C_{1-\alpha,n}^{s}$ is asymptotically valid for $\rho_n$ by (-@eq-sign-specific-fieller-coverage).
...
To see why $I_{1-\alpha,n}^{s}$ is uniformly valid, define the inclusion event
$$
E^{\mathrm{Fieller},s}
=
\left\{
...
Thus
$$
\mathbb{P}_{\boldsymbol\theta}\{\rho_n \notin I_{1-\alpha,n}^{s}\}
\leq
\mathbb{P}_{\boldsymbol\theta}\{(E^{\mathrm{Fieller},s})^c\}
+
\mathbb{P}_{\boldsymbol\theta}\left\{T_n^{\mathrm{ATE}}<-z_{1-\alpha^{\mathrm{ATE}}/2}\right\}.
$$
...
\right\} \\
&=
\mathbb{P}_{\boldsymbol\theta}\{\rho_n^s \notin C_{1-\alpha^{\mathrm{Fieller}},n}^{s}\}
+
\mathbb{P}_{\boldsymbol\theta}\left\{
...
$$
\begin{aligned}
\liminf_{n\to\infty}\inf_{\boldsymbol\theta \in \Theta}\mathbb{P}_{\boldsymbol\theta}\{\rho_n \in I_{1-\alpha,n}^{s}\}
&\geq
...
$$
\begin{aligned}
\mathbb{P}_{\boldsymbol\theta}\left(\rho_n\in I_{1-\alpha,n}\right)
&\geq
\mathbb{P}_{\boldsymbol\theta}\left((E_0^+\cap E^{\mathrm{Fieller},+})\cup E_0^0\right) \\[1ex]
&=
\mathbb{P}_{\boldsymbol\theta}\left((E_0^-)^c \cap (E_0^0\cup E^{\mathrm{Fieller},+})\right) \\[1ex]
&=
1-\mathbb{P}_{\boldsymbol\theta}\left(E_0^-\cup ((E_0^0)^c\cap (E^{\mathrm{Fieller},+})^c)\right) \\[1ex]
&\geq
1-\mathbb{P}_{\boldsymbol\theta}(E_0^-)-\mathbb{P}_{\boldsymbol\theta}((E^{\mathrm{Fieller},+})^c).
\end{aligned}
...
Therefore, for every $n$ and $\boldsymbol\theta\in\Theta$,
...
\right)
+
\max \left\{\mathbb{P}_{\boldsymbol\theta}(\rho_n^{+} \notin C_{1-\alpha^{\mathrm{Fieller}},n}^{+}), \mathbb{P}_{\boldsymbol\theta}(\rho_n^{-} \notin C_{1-\alpha^{\mathrm{Fieller}},n}^{-})\right\}.
\end{aligned}
$$

Taking the supremum over $\boldsymbol\theta\in\Theta$ gives
...
\right)
+
\max \left\{\sup_{\boldsymbol\theta\in\Theta}\mathbb{P}_{\boldsymbol\theta}(\rho_n^{+} \notin C_{1-\alpha^{\mathrm{Fieller}},n}^{+}), \sup_{\boldsymbol\theta\in\Theta}\mathbb{P}_{\boldsymbol\theta}(\rho_n^{-} \notin C_{1-\alpha^{\mathrm{Fieller}},n}^{-})\right\}.
\end{aligned}
$$
...
@fig-uniform-noncoverage recreates the bottom panels of @fig-scaled-ate-noncoverage and @fig-lift-skewness, but reports non-coverage for the uniformly-valid confidence interval in (-@eq-final-confidence-set).
In the top panel, the lift varies while the baseline is fixed at $\overline{y}_{0,n}=1$.
...
Let $m_n^{\mathrm{untreated}}$ denote the number of units outside the experiment, and let $\overline{y}_n^{\mathrm{untreated}}$ denote their mean outcome.
...
$$
\overline{y}^\mathrm{full}_{d,n} = \frac{n}{m_n^{\mathrm{untreated}} + n}\overline{y}_{d, n} + \frac{m_n^{\mathrm{untreated}}}{m_n^{\mathrm{untreated}} + n} \overline{y}^{\mathrm{untreated}}_n
$$

The full-population average treatment effect is defined as

$$
\psi_n^{\mathrm{full}} = \overline{y}^\mathrm{full}_{1,n} - \overline{y}^\mathrm{full}_{0,n} = \frac{n}{m_n^{\mathrm{untreated}} + n} \psi_n^{\mathrm{ATE}}
$$
...
$$
q_n = \frac{m_n^{\mathrm{untreated}}}{n},
\qquad
b_n = \overline{y}_{0,n}+q_n\overline{y}_n^{\mathrm{untreated}},
\qquad
\widehat b_n = \widehat{y}_{0,n}+q_n\overline{y}_n^{\mathrm{untreated}}.
$$

Dividing the numerator and denominator in (-@eq-full-relative-effect-definition) by $n/(m_n^{\mathrm{untreated}}+n)$ gives
$$
...
$$
\sqrt n(\widehat b_n-b_n)=\sqrt n(\widehat{y}_{0,n}-\overline{y}_{0,n}),
$$
...
For any candidate value $r$, the sample analogue simplifies to $\widehat b_n r-m^s\widehat{\psi}_n^{\mathrm{ATE}}$.
...
\left\{
r:
\left|\widehat b_n r-m^s\widehat{\psi}_n^{\mathrm{ATE}}\right|
\leq
...
C_{1-\alpha,n}^{\mathrm{full},s}
= \Bigg\{r :{}&
\left(\widehat b_n^2 - \frac{z_{1-\alpha/2}^2}{n}\,\widehat{\sigma}_{0,n}^2\right)r^2 \\
&\qquad - 2m^s\left(\widehat b_n\,\widehat{\psi}_n^{\mathrm{ATE}} + \frac{z_{1-\alpha/2}^2}{n}\,\widehat{\sigma}_{0,n}^2\right)r
- \frac{2z_{1-\alpha/2}^2}{n}\,\widehat{\sigma}_{0,n}\,\widehat{\sigma}_{1,n}\left(\left|1+m^sr\right| - 1\right) \\
...
These confidence sets have the same form as the confidence sets in (-@eq-fieller-sets), with $\widehat b_n$ replacing $\widehat{y}_{0,n}$.
...
T_{0,n}^{\mathrm{full}}
=
\frac{\sqrt n\,\widehat b_n}{\widehat{\sigma}_{0,n}}.
$$
...
The proof of uniform validity is the same as the proof for (-@eq-final-confidence-set), with $\widehat{y}_{0,n}$ replaced by $\widehat b_n$.
The sign-pretest error is controlled because $\widehat b_n-b_n=\widehat{y}_{0,n}-\overline{y}_{0,n}$, and the sign-specific sets in (-@eq-full-fieller-sets) are valid confidence sets for $\psi_n^{\mathrm{full},s}$.
Thus, over parameter sequences with $b_n\neq0$,

$$
\liminf_{n\to\infty}
\inf_{\boldsymbol\theta\in\Theta:\,b_n\neq0}
\mathbb{P}_{\boldsymbol\theta}\left(
...
Throughout this section, assume that $|T_{0,n}|> z_{1-\alpha/2}$, which implies $G_n < 1$.
...
\right| \\
&\leq H_n+G_n\left|r-\widehat{\rho}_n\right|.
\end{aligned}
$$
...
$$
\left|r-\widehat{\rho}_n\right|\leq \frac{H_n}{1-G_n}.
$$
...
\subseteq
\left\{
r: \left|r-\widehat{\rho}_n\right|\leq\frac{H_n}{1-G_n}
\right\}.
...
\widehat{\sigma}_{1,n}
\right) \\
&\qquad= H_n-G_n\left|r-\widehat{\rho}_n\right|.
\end{aligned}
$$

If $\left|r-\widehat{\rho}_n\right|\leq H_n/(1+G_n)$, then $\left|r-\widehat{\rho}_n\right|\leq H_n-G_n\left|r-\widehat{\rho}_n\right|$.
We therefore have
...
Since the delta-method confidence interval can be written as $\{r: |r-\widehat{\rho}_n|\leq H_n\}$, (-@eq-fieller-delta-outer-inclusion) and (-@eq-fieller-delta-inner-inclusion) imply that the sign-specific confidence set is squeezed between a contraction and expansion of the delta-method confidence interval.
Therefore,
