...
This note develops confidence intervals for linear combinations of treatment group means in randomized experiments. We use a fixed potential-outcomes framework and establish uniform coverage properties, and then illustrate the behavior of the confidence intervals in a Monte Carlo simulation.
...
Also define the centered potential-outcomes

$$
u_{i, 0, n} = y_{i,0} - \overline{y}_{0,n}, \qquad
u_{i, 1, n} = y_{i,1} - \overline{y}_{1,n}, \qquad
\mathbf{u}_{i, n} =
\begin{pmatrix}
u_{i,0,n} \\
u_{i,1,n}
...
the potential-outcome variances

$$
S_{0,n}^2
=
\frac{1}{n}\sum_{i=1}^n u_{i,0,n}^2, \qquad
S_{1,n}^2
=
\frac{1}{n}\sum_{i=1}^n u_{i,1,n}^2
$$
...
**Assumption 2.** The observed outcome $Y_i$ is determined by
...
Assumption 3(i) rules out dominant units by requiring that no single unit accounts for a non-negligible share of potential-outcome variation, while Assumption 3(ii) ensures the [Berry--Esseen bound](https://en.wikipedia.org/wiki/Berry%E2%80%93Esseen_theorem) for the target contrast vanishes uniformly over $\Theta$.
In the [Appendix](#sec-stationary-ergodic-conditions), we show that if candidate potential-outcome paths are modeled as realizations of a stationary ergodic process, then for any $\epsilon>0$ there is a set of potential-outcome paths occurring with probability at least $1-\epsilon$ such that Assumption 3 holds after restricting the potential-outcome paths in $\Theta$ to that set.
...
We estimate $\psi_n$ by replacing the potential-outcome means with the control and treatment sample means. Define

$$
\begin{gathered}
\widehat{y}_{0,n} = \frac{\overline{Y(1 - D)}_n}{1 - \overline{D}_n}, \qquad \widehat{y}_{1,n} = \frac{\overline{YD}_n}{\overline{D}_n}, \qquad \widehat{\mathbf{y}}_n = \begin{pmatrix} \widehat{y}_{0,n} \\ \widehat{y}_{1,n} \end{pmatrix}
\end{gathered}
$$ {#eq-estimators}
...
The estimator variance is much lower than the bound, and the displayed intervals cover $\psi_n$ in all displayed realizations.
...
Let $D_{i,1}=D_i$, $D_{i,0}=1-D_i$, $p_1=p$, $p_0=1-p$, and
